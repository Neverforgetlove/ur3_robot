...
ur3_controller_server::ur3_controller_server(ros::NodeHandle &n, string arm_topic, string gripper_topic):

    node(n),
    client(gripper_topic,true),
    server(node,"/ur3_controller", 
    boost::bind(&ur3_controller_server::goalCB, this, _1),
    boost::bind(&ur3_controller_server::cancelCB, this, _1),
    false),
    has_active_goal(false)
    {
        pub_joint_command = node.advertise<trajectory_msgs::JointTrajectory>(arm_topic, 1000);
        execute_timer  = node.createTimer(ros::Duration(0.0333333), &ur3_controller_server::listenState, this);
        watchdog_timer = node.createTimer(ros::Duration(1.0), &ur3_controller_server::watchdog, this);
        server.start();   
    };

ur3_controller_server::~ur3_controller_server(){
...
void ur3_controller_server::listenState(const ros::TimerEvent &e){
    if(!has_active_goal && !goal_queue.empty()){
        has_active_goal = true;
        std::thread exec_thread(std::bind(&ur3_controller_server::serverExecute,this));
        exec_thread.detach();
    }
};

void ur3_controller_server::judgeAbort(){
    if(has_active_goal){
...
void ur3_controller_server::serverExecute(){
    if(!goal_queue.empty()){
        ...
        if(!cancel_queue.empty() && (gh == cancel_queue.top())){
            ...
            duration_time += 0.033333333;   //容错30Hz
            //当前运行目标
            ...
            vector<ur3_msgs::ControlStep> current_csp = gh.getGoal()->csp;
                ...
                if(!current_csp[i].barm && !current_csp[i].bgrasp){
                    ...
                    return ;
                        ...
                        if(current_csp[i].barm){
                            ...
                            if(current_csp[i].joint_trajectory.joint_trajectory.points.size())
                            wait_time = current_csp[i].joint_trajectory.joint_trajectory.points.back().time_from_start.toSec();
                    }
                    if(current_csp[i].bgrasp){
                        if(!GripperCommand(current_csp[i].on_off)){
                            ROS_ERROR("Grasp Command Failed!!");
                            Stop();
                            return ;
                        }
                    }
...
void ur3_controller_server::watchdog(const ros::TimerEvent &e) {
    judgeAbort();
};

void ur3_controller_server::Stop(){
...
void ur3_controller_server::goalCB(GoalHandle gh){
    goal_queue.push(gh);
    ROS_INFO("Goal pushed into the queue!"); 
    gh.setAccepted();
};

void ur3_controller_server::cancelCB(GoalHandle gh){    
    if (active_goal == gh){
        Stop();
    }else{
...
void ur3_controller_server::ActiveCb(){
    ROS_INFO("Goal just went active");
};

void ur3_controller_server::DoneCb(const actionlib::SimpleClientGoalState& state,
...
void ur3_controller_server::FeedbackCb(
...
bool ur3_controller_server::GripperCommand(bool on_off) {
    is_grasped = true;
    double target_width, max_effort;
    if(!(node.getParam("gripper_effort", max_effort))){
    ...
    client.sendGoal(goal,
        boost::bind(&ur3_controller_server::DoneCb, this, _1, _2),
        boost::bind(&ur3_controller_server::ActiveCb, this),
        boost::bind(&ur3_controller_server::FeedbackCb, this, _1));
    return is_grasped;
};

void ur3_controller_server::JointCommand(trajectory_msgs::JointTrajectory trajectory) {
