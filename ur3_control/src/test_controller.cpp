...
typedef actionlib::SimpleActionClient<ur3_msgs::BaseControlAction> Client;
...
int main(int argc, char **argv)
{
    ...
    spinner.start();
	Client client("/ur3_controller", true);
    ur3_msgs::BaseControlGoal goal;
    vector<ur3_msgs::ControlStep>  step_list;
    const double degree = 3.1415926/180;
   // message declarations
    std::vector<trajectory_msgs::JointTrajectoryPoint> points_n(1);    
    step_list.resize(1);
    nh.getParam("ur3_controller/joints",step_list[0].joint_trajectory.joint_trajectory.joint_names);
    points_n[0].positions.resize(6);
